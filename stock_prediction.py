...
#------------------------------------------------------------------------------
#Task 2C
def load_data(data_dir = "stock_data", DATA_SOURCE = "yahoo", COMPANY = 'CBA.AX', TRAIN_START = dt.datetime(2020,01,01), TRAIN_END = dt.datetime(2023,08,01), data_file_name = "stock_data",
              features = ["Open", "Close", "Low", "High"], PREDICTION_DAYS = 60):
    ...
    # To store the training and testing data
    x = []
    y = []
...
def bloxplot_chart(data, window_size = 20):
    close_prices = data['Close']
    box_data = []
    for i in range(len(close_prices) - window_size + 1):
        window = close_prices.iloc[i:i+window_size]
        box_data.append(window.values)
    plt.figure(figsize=(14,6))
    plt.boxplot(box_data, showfliers=True)
    plt.title(f'Boxplot of Closing Prices ({window_size}-Day Moving Window)')
